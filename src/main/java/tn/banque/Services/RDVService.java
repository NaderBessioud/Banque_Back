...
public class RDVService {
	...
	public RDV addRDV(RDV r,long id,long idc) {
		User conseiller=userRepo.findById(id).get();
		User client=userRepo.findById(idc).get();
		SimpleDateFormat fd=new SimpleDateFormat("dd-MM-yyyy"); 
		
		Notifications notifications=new Notifications("Rendez-vous","Vous avez un nouveau RDV avec "+client.getNom()+" "+
		client.getPrenom()+" le "+fd.format(r.getDaterdv())+" à"+r.getHeure()+" h",false,new Date());
		notifications.setUser(conseiller);
		notifications=notifiactionsService.addNotification(notifications);
	...
	public RDV getClientRDVByDate(long idc,Date date) {
		User client=userRepo.findById(idc).get();
		return rdvRepository.findByClientAndDaterdv(client, date);
	}
	
	public float getRDVFrais(long idc) {
		User client=userRepo.findById(idc).get();
		int nbr=rdvRepository.findByClient(client).size();
		float frais=0;
		if( nbr<3 && 0<nbr)
			frais=20;
		else if(3<=nbr && nbr<10)
			frais=15;
		else
			frais=10;
		return frais;
	...
	public List<User> getEmployeeRDV(long id){
		
		List<User> result=repo.getEmployeeRDV(id);
		return result;
	}
...
public User getConseillerProche(double lat,double lon) throws ClientProtocolException, IOException {
		...
		List<User> conseillers=userRepo.findByRoleAndConge(TypeEmployee.CONSEILLER, false);
